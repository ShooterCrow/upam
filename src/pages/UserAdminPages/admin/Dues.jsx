...
import { useGetDuesQuery } from "./duesApiSlice";
import { List, User as UserIcon, Loader2, ClipboardList } from "lucide-react";
...
const Dues = () => {
  ...
  const { data: response, isLoading, isFetching, isError, error, refetch } = useGetDuesQuery({
    page,
    limit,
  });
  ...
  if (isLoading && !dues.length) {
    return (
      <div className="h-[70vh] flex items-center justify-center">
        <LoadingState message="Fetching dues..." />
      </div>
    );
  }
  ...
  if (isError) {
    return (
      <div className="h-[70vh] flex items-center justify-center">
        <ErrorState
          message={error?.data?.message || "Could not load dues"}
          onRetry={refetch}
        />
      </div>
    );
    ...
    <div className="animate-in fade-in duration-500 pb-10">
      ...
      <div className="bg-white px-6 py-4 border-b border-gray-100 flex flex-col md:flex-row justify-between items-center gap-4">
        <div className="flex items-center gap-3">
            ...
            <h2 className="text-lg font-bold text-slate-800">Dues Management ({totalDues})</h2>
        </div>
        {/* We can add an import or add button here later */}
      </div>
            ...
            <tbody className="divide-y divide-slate-50 whitespace-nowrap relative">
              {dues.map((due) => (
                <tr key={due._id} className="group hover:bg-slate-50 transition-colors">
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <p className="text-sm font-semibold text-slate-700">
                          {due.user ? `${due.user.firstName} ${due.user.lastName}` : "Unknown User"}
                        </p>
                        <p className="text-xs text-slate-400">{due.user?.email || "No email provided"}</p>
                      </div>
                    </div>
                  </td>
                  ...
                  <td className="px-6 py-5">
                    <span className="text-sm text-slate-600">{due.due_yr}</span>
                  </td>
